...
public class SecurityConfig {
    @Bean public SecurityFilterChain filterChain(HttpSecurity http, InternalApiKeyFilter internal) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable)
                .authorizeHttpRequests(a->a.requestMatchers("/actuator/**","/v3/api-docs/**","/swagger-ui.html","/swagger-ui/**").permitAll().requestMatchers("/applications/**").hasAnyRole("USER","MANAGER","ADMIN").anyRequest().authenticated())
    ...
    @Bean public InternalApiKeyFilter internalApiKeyFilter(@Value("${security.internal-api-key:}") String apiKey) { return new InternalApiKeyFilter(apiKey, "ROLE_USER"); }
