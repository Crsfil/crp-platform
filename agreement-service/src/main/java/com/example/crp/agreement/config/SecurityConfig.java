...
public class SecurityConfig {
    @Bean public SecurityFilterChain filterChain(HttpSecurity http, ObjectProvider<InternalApiKeyFilter> internal) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable)
                .authorizeHttpRequests(a -> a
                        ...
                        .requestMatchers(HttpMethod.GET, "/agreements/**")
                        .access(new WebExpressionAuthorizationManager("hasAnyRole('MANAGER','ADMIN') or @trustedClientAuthorizer.isTrusted(authentication)"))
                        .requestMatchers("/agreements/**").hasAnyRole("MANAGER","ADMIN")
    ...
    public InternalApiKeyFilter internalApiKeyFilter(@Value("${security.internal-api-key:}") String apiKey) { return new InternalApiKeyFilter(apiKey, "ROLE_MANAGER"); }

    static class InternalApiKeyFilter extends OncePerRequestFilter {
        private final String apiKey; private final String role;
        InternalApiKeyFilter(String apiKey, String role) { this.apiKey = apiKey; this.role = role; }
